...
class ORMFixture:
    ...
    def convert_groups_to_model(self, groups):
        def convert(group):
            return Group(id=str(group.id), gr_name=group.name, gr_header=group.header, gr_footer=group.footer)
    ...
    @db_session
    def get_cont_in_group(self, group):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.id))[0]
        return self.convert_cont_to_model(orm_group.contacts)

    @db_session
    def get_cont_not_in_group(self, group):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.id))[0]
